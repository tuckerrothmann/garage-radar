...
class TestListingsEndpoint:
    def _client(self, listings: list, total: int = None):
        session = AsyncMock()
        # First call: count; subsequent calls: page fetch + cluster lookup per listing
        session.scalar = AsyncMock(side_effect=[total or len(listings)] + [None] * len(listings))
        result = MagicMock()
