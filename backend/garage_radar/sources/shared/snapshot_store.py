...
class SnapshotStore:
    ...
    def write(self, raw: RawPage) -> Optional[str]:
        ...
        try:
            ...
            fallback_root = Path(tempfile.gettempdir()) / "garage-radar-snapshots"
            fallback_path = (
                fallback_root
                / raw.source
                / raw.fetched_at.strftime("%Y-%m-%d")
                / f"{_url_to_slug(raw.url)}.{ 'json' if raw.content_type == 'json' else 'html' }"
            )
            try:
                fallback_path.parent.mkdir(parents=True, exist_ok=True)
                fallback_path.write_text(raw.content, encoding="utf-8")
                logger.warning(
                    "Snapshot path %s was not writable for %s; wrote fallback snapshot to %s",
                    self.base_path,
                    raw.url,
                    fallback_path,
                )
                ...
                logger.warning(
                    "Failed to write snapshot for %s to fallback path after permission error: %s",
                    raw.url,
                    fallback_exc,
                )
                return None
