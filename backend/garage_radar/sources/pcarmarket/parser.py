...
_SOLD_TOKENS = ("sold", "winning bid", "winner", "final price", "hammer price")
_NO_SALE_TOKENS = ("reserve not met", "bid to", "withdrawn", "no sale", "not sold", "passed")
...
class PcarmarketParser(BaseParser):
    ...
    def _parse(self, raw: RawPage, as_comp: bool) -> ParsedListing | None:
        ...
        auction_end_at = self._extract_end_at(soup)
        ...
        return ParsedListing(
            ...
            time_remaining_text=self._extract_time_remaining_text(soup) if not is_completed else None,
    ...
    def _extract_title(self, soup: BeautifulSoup, url: str | None = None) -> str | None:
        ...
        for meta_selector, attr_name in (("meta[property='og:title']", "content"), ("meta[name='twitter:title']", "content")):
            el = soup.select_one(meta_selector)
            if el and el.get(attr_name):
                title = str(el.get(attr_name)).split(" | ", 1)[0].strip()
    ...
    def _extract_result(self, soup: BeautifulSoup) -> tuple[bool, float | None, str | None]:
        """Returns (is_completed, final_price, sale_date_str)."""
        # Dedicated sold-result price elements.
        for sel in [".sold-price", ".final-price", ".hammer-price"]:
            el = soup.select_one(sel)
            if el:
        ...
        # General result/status badges can represent either sold or no-sale outcomes.
        for sel in [".auction-result", ".auction-status", ".listing-status", ".badge"]:
            el = soup.select_one(sel)
            if not el:
                continue

            text = " ".join(el.get_text(" ", strip=True).lower().split())
            if any(token in text for token in _NO_SALE_TOKENS):
                return False, None, self._extract_end_date(soup)
    ...
    def _extract_end_date(self, soup: BeautifulSoup) -> str | None:
        end_at = self._extract_end_at(soup)
        if end_at:
            return end_at[:10]
        return None

    def _extract_end_at(self, soup: BeautifulSoup) -> str | None:
    ...
    def _extract_time_remaining_text(self, soup: BeautifulSoup) -> str | None:
        for sel in [".countdown", ".time-left", ".auction-countdown"]:
            el = soup.select_one(sel)
            if el:
    ...
    def _is_generic_title(self, title: str) -> bool:
        lowered = " ".join(title.casefold().split())
        return "auctions & marketplace for collectible vehicles" in lowered or lowered in {
            "pcarmarket",
            "pcarmarket auctions",
        }
    ...
    def _spec(self, soup: BeautifulSoup, names: list[str]) -> str | None:
        """
