...
class EbayParser(BaseParser):
    ...
    def parse_listing(self, raw: RawPage) -> ParsedListing | None:
        ...
        parsed = ParsedListing(
            ...
            current_bid=self._extract_price(item),
            ...
            bidder_count=self._extract_bidder_count(item),
            is_completed=is_completed,
            listing_date=self._extract_listing_date(item),
            auction_end_at=self._extract_end_at(item),
            time_remaining_text=self._extract_time_remaining(item, raw.fetched_at),
        )
        return parsed
    ...
    def _load_item(self, raw: RawPage) -> dict | None:
        if not raw.content:
            return None
        try:
    ...
    def _extract_year(self, item: dict) -> int | None:
        ...
        for field_name in ("Year", "Model Year", "Year of Manufacture"):
            year_str = self._spec_value(item, field_name)
            if year_str:
                try:
                    return int(year_str)
                except ValueError:
    ...
    def _extract_mileage(self, item: dict) -> int | None:
        for field_name in ("Mileage", "Odometer"):
            mileage_str = self._spec_value(item, field_name)
            if mileage_str:
                clean = _PRICE_CLEAN_RE.sub("", str(mileage_str).split()[0])
                try:
                    return int(clean)
                except ValueError:
                    pass
    ...
    def _extract_vin_from_specifics(self, item: dict) -> str | None:
        for field_name in ("VIN", "Vehicle Identification Number (VIN)"):
            vin = self._spec_value(item, field_name)
            if vin and _VIN_RE.match(vin.upper()):
                return vin.upper()
        return None

    def _extract_bidder_count(self, item: dict) -> int | None:
        for path in [
    ...
    def _spec_value(self, item: dict, name: str) -> str | None:
    ...
    def _extract_listing_date(self, item: dict) -> str | None:
        start = item.get("StartTime") or item.get("ListingDetails", {}).get("StartTime")
        if start:
            return start[:10]
        return None

    def _extract_end_date(self, item: dict) -> str | None:
        end = self._extract_end_at(item)
        if end:
            return end[:10]
        return None

    def _extract_end_at(self, item: dict) -> str | None:
        for path in [
            ("EndTime",),
    ...
    def _extract_time_remaining(self, item: dict, fetched_at: datetime | None) -> str | None:
        for path in [
            ...
            if isinstance(val, str) and val.strip():
                human = self._humanize_time_left(val.strip())
                if human:
                    return human
                return val.strip()

        end_at = self._extract_end_at(item)
        if not (end_at and fetched_at):
            return None
        try:
            end_dt = datetime.fromisoformat(end_at.replace("Z", "+00:00"))
        except ValueError:
            return None
        fetched = fetched_at if fetched_at.tzinfo else fetched_at.replace(tzinfo=UTC)
        delta = end_dt - fetched.astimezone(UTC)
        if delta.total_seconds() <= 0:
            return "ended"
        total_minutes = int(delta.total_seconds() // 60)
        days, remainder_minutes = divmod(total_minutes, 60 * 24)
        hours, minutes = divmod(remainder_minutes, 60)
        return self._format_duration(days=days, hours=hours, minutes=minutes)

    def _humanize_time_left(self, value: str) -> str | None:
        match = _TIME_LEFT_RE.match(value)
        if not match:
            return None
        parts = {name: int(amount) if amount else 0 for name, amount in match.groupdict().items()}
