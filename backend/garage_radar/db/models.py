...
Originally built for air-cooled Porsche 911s. The schema now keeps the
Porsche-specific generation fields, but the vehicle identity layer is
generalized so other makes and models can flow through the same pipeline.
"""
...
class Listing(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("year BETWEEN 1886 AND 2100", name="ck_listings_year_range"),
    ...
    # Price
    current_bid: Mapped[Optional[float]] = mapped_column(Numeric(10, 2), nullable=True)
    ...
    auction_end_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True),
        nullable=True,
    )
    time_remaining_text: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
...
class Comp(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("year BETWEEN 1886 AND 2100", name="ck_comps_year_range"),
        Index("idx_comps_sale_date", "sale_date"),
        Index("idx_comps_make_model", "make", "model"),
...
class CompCluster(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            ...
            name="ck_comp_clusters_year_bucket_range",
        ),
        Index(
    ...
    year_bucket: Mapped[Optional[int]] = mapped_column(
