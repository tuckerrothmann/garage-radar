...
_JOBS = [
    # (job_id, func, args, kwargs, cron_kwargs)
    (
        "crawl_bat",
        crawl_job,
        ("bat",),
        {"max_pages": 15, "include_sold": True, "max_urls": 5},
        {"hour": 2, "minute": 0},
    ),
    (
        "crawl_carsandbids",
    ...
    (
        ...
        crawl_job,
        ("ebay",),
        {"max_pages": 5, "include_sold": True, "max_urls": 5},
        {"hour": 4, "minute": 0},
    ),
    (
        "crawl_pcarmarket",
        crawl_job,
        ("pcarmarket",),
        {"max_pages": 10, "include_sold": True, "max_urls": 5},
        {"hour": 4, "minute": 45},
    ),
    (
        "insights",
...
def _register_jobs(scheduler: AsyncIOScheduler) -> None:
    ...
    for job_id, func, job_args, job_kwargs, cron_kwargs in _JOBS:
        scheduler.add_job(
            func,
            args=job_args,
            kwargs=job_kwargs,
...
async def run_scheduler_forever() -> None:
    """Run the standalone scheduler inside an active asyncio event loop."""
...
def _run_now_and_exit(job_name: str) -> None:
    ...
    async def _run():
        if job_name in ("bat", "carsandbids", "ebay", "pcarmarket"):
            result = await crawl_job(job_name, max_pages=5, include_sold=True, max_urls=5)
